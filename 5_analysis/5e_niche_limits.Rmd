...
```{r}
dataset_final <- readxl::read_xlsx(here("./data/data_1/dataset_final_12_02_26.xlsx"))
...
```{r}
# Input directory
directory_path <- here("./data/niche_limits")
...
segments$label <- with(segments, paste0(
  label, "[",
...
d1 <- ggplot(combined_data, aes(x = bio5, fill = group)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 8, colour = "black", hjust = 0),
...
d2 <- ggplot(combined_data, aes(x = bio6, fill = group)) +
  ...
  scale_y_reverse(
    ...
    minor_breaks = setdiff(
      seq(0,200000, by = 10000),
      c(0, 50000, 100000, 150000, 200000)),
    guide = guide_axis(minor.ticks = TRUE)
    ) +
...
p1_short <- ggplot() +
  ...
  # ) +
  geom_segment(
    data = segments %>% filter(as.numeric(model) <= 100),
    aes(
      yend = model,
      y = model,
      x = median_bio6,
  ...
  geom_density_ridges(
    data = all_data_limits_long %>% filter(as.numeric(model) <= 100),
    aes(
      x = value,
      y = model,
...
guides(
  ...
  theme(
    ...
    axis.minor.ticks.x.top = element_line(colour = "black", linewidth = mm(0.15))
  )
```
...
plot <- d1 + p1_short + p1_full +
  plot_layout(design = layout)
plot

ggsave(plot = plot, filename = "Fig5_thermal_limits.pdf", path = here("figures"), width = 180, height = 200, device = "pdf", units = "mm")
```
...
p1_full_supp <- ggplot() +
  ...
  geom_segment(
    data = segments %>% filter(as.numeric(model) <= 250),
    aes(
      yend = model,
      y = model,
      x = median_bio6,
  ...
  geom_density_ridges(
    data = all_data_limits_long %>% filter(as.numeric(model) <= 250),
    aes(
      x = value,
      y = model,
      group = interaction(model, Intercept),
...
plot <- d1 + p1_full_supp + d2 +
  plot_layout(design = layout)
plot

ggsave(plot = plot, filename = "FigS7_thermal_limits.pdf", path = here("figures"), width = 180, height = 380, device = "pdf", units = "mm")
```
...
pleistocene_data$label <- str_c(
  pleistocene_data$label,
  "~",
  "(",
...
checklist_filtered <- checklist %>%
  filter(genus %in% pleistocene_data$genus) %>%
...
plot <- ggplot() +
  geom_segment(data = pleistocene_data, aes(y = genus, yend = genus, x = min_ma, xend = max_ma, color = factor(status)), size = 3) +
...
plot
ggsave(plot = plot, filename = "Fig3.pdf", path = here("figures"), width = 140, height = 120, device = "pdf", units = "mm", bg = "white")
```

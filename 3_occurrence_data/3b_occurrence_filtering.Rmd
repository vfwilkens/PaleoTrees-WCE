...
```{r, eval = FALSE}
occurrence_data_path <- here("./data/data_3/occurrence_data/") # Input path (output path from 2a_occurrence_data)
output_path <- here("./data/data_3/occurrence_data_filtered/") # Output path for the thinned .csv file of each genus

thinning_grid <- rast(here("./data/data_2/bioclim_current_global_0_25deg.tif"))
...
for (g in 1:length(all_genera)) {
  ...
  for (s in 1:length(species_list)) {
    ...
    thinned_index <- GeoThinneR::grid_thinning(occurrences[, c("decimalLongitude", "decimalLatitude")], raster_obj = thinning_grid)

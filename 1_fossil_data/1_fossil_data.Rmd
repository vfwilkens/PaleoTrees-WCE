...
pbdb_data <- read.csv(here("./data/data_1/pbdb_data_12_02_2026.csv"), skip = 18)
...
literature_data <- read_xlsx(here("./data/data_1/literature_records_12_02_26.xlsx"))
...
checklist <- wcvp_checklist(
  taxon_rank = "genus", synonyms = FALSE, native = TRUE,
...
# To download the GlobalTreeSearch database checklist: https://tools.bgci.org/global_tree_search.php
...
checklist_filtered <- checklist %>%
  filter(area_code_l3 %in% WCE_botanical_countries) %>%
  filter(taxon_status == "Accepted") %>%
...
native_genera <- subset(native_genera, native_genera %in% dataset_final$genus)
...
write_xlsx(dataset_final, here("./data/data_1/dataset_final_12_02_26.xlsx"))
...
```{r}
literature_references <- read_xlsx(here("./data/data_1/literature_records_12_02_26.xlsx"))
literature_references <- literature_references[-(1:2), c(1, 12:13)]
colnames(literature_references) <- c("genus", "notes", "references")

dashboard_data <- dataset_final

epoch_cols <- c("Paleocene", "Eocene", "Oligocene", "Miocene", "Pliocene")

dashboard_data <- dashboard_data %>%
  left_join(pbdb_dataset, by = "genus", suffix = c("", "_pbdb")) %>%
  mutate(
    Paleocene = ifelse(!is.na(Paleocene_pbdb) & Paleocene_pbdb == 1, 2, Paleocene),
    Eocene    = ifelse(!is.na(Eocene_pbdb) & Eocene_pbdb == 1, 2, Eocene),
    Oligocene = ifelse(!is.na(Oligocene_pbdb) & Oligocene_pbdb == 1, 2, Oligocene),
    Miocene   = ifelse(!is.na(Miocene_pbdb) & Miocene_pbdb == 1, 2, Miocene),
    Pliocene  = ifelse(!is.na(Pliocene_pbdb) & Pliocene_pbdb == 1, 2, Pliocene)
  ) %>%
  select(-ends_with("_pbdb"))


family_data <- trees_final %>%
  select(genus, family) %>%
  distinct()

dashboard_data <- dashboard_data %>%
  left_join(family_data, by = "genus") %>%
  relocate(family, .after = genus)


dashboard_data <- dashboard_data %>%
  left_join(literature_references, by = "genus")


head(dashboard_data)

write_xlsx(dashboard_data, here("./data/data_1/dashboard_data_12_02_26.xlsx"))
```